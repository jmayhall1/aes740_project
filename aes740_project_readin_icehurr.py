...
data_dbz = np.array(netCDF4.Dataset(path).variables.get('dbz'))
data_qi = np.array(netCDF4.Dataset(path).variables.get('qi'))
...
for i in range(data_qv.shape[0]):
    ...
    fig = mlab.figure(size=(1024, 1024))
    s = mlab.contour3d(tke_data, contours=10, colormap='Greys')
    v = mlab.contour3d(qi_data, contours=25, colormap='jet', opacity=0.5)
    mlab.axes(xlabel='x', ylabel='y', zlabel='z')
    mlab.outline(s)
    mlab.savefig(f'C:/Users/jmayhall/Downloads/aes740_project/ice_photos/qi/qi_tke_timestep{i}.png')
    mlab.close(all=True)
    mlab.clf()
